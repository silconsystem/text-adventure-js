...
let inv = () => {
  ...
  // write to div
  //const displayInv = document.querySelector('#inventory');
  
  // html content
  //let inventoryHTML = '';
  
  // item name and img URL
  let itemList = [];
  let itemImg = [];

  if (!items.length) {
    inventoryContainer.innerHTML = `You don't have any items in your inventory.`;
...
//    itemList.push(getName(item.name));
//    itemImg.push(getItem(item.name).imgUrl);
    
//    inventoryHTML += `<img id="item-icon" class="child::first-child" src="${getItem(item.name).imgUrl}"><span>${item.name}</span><br>`;
    
    updateInventoryPage();
    
    console.log(bullet, itemList);
  });
  
  toggleVisibility("id", "inventory-nav", "block");
...
let enterRoom = (id) => {
  ...
  // display graphics
  displayImageByRoomId(room.imgUrl);
